...
const validateForm = () => {
  ...
  const updatedCountry = {
    ...props.country,
    name: {
      official: form.value.officialName,
      common: form.value.commonName
    },
    area: Number(form.value.area),
    population: Number(form.value.population),
    region: form.value.region,
    capital: [form.value.capital],
    languages: { [form.value.language]: form.value.language },
    currencies: { [form.value.currencies]: { name: form.value.currencies } },
...
watch(() => props.isOpen, (val) => {
  if (val && props.country) {
    ...
    form.value = {
      ...
      language: Object.values(props.country.languages || {})[0] || '',
      currencies: Object.keys(props.country.currencies || {})[0] || '',
  ...
  >
    ...
    <span class="text-sm text-gray-500">Upload flag (PNG ou JPG)</span>
                  ...
                  <div class="flex items-center space-x-2 justify-end p-4 md:p-5 border-t border-gray-200 rounded-b dark:border-gray-600">
                    ...
                    <button @click="saveEdit" data-modal-hide="default-modal" type="button" class="cursor-pointer text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">Save Changes</button>
                  </div>
              </div>
          </div>
      </div>
